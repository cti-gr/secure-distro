...
function loadPageMods() {
	...
	SDK.pageMod.PageMod({
		...
		onAttach: function (worker) {
			...
			worker.port.on('record_install', function () {
				metrics.recordInstall();
			});
	...
	// ghostery_dot_com.js
	SDK.pageMod.PageMod({
		include: ['*.ghostery.com', '*.ghostery.com/download', 'https://www.ghostery.com/apps/*'],
		contentScriptWhen: 'end',
		attachTo: ['existing', 'top'],
		...
		contentStyleFile: [
			SDK.self.data.url('lib/vendor/bootstrap/bootstrap.css'),
		...
		onAttach: function (worker) {
			worker.port.on('appsPageLoaded', function (msg) {
				worker.port.emit('appsPageData', { blocked: conf.selected_app_ids[msg.id] == 1 });
			});
			worker.port.on('panelSelectedAppsUpdate', function (message) {
				if (message.app_selected) {
					conf.selected_app_ids[message.app_id] = 1;
